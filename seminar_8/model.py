...
from view import (get_employee, get_changes, get_id, get_first_name, get_last_name, show_search_menu,
                  get_phone_number, get_position, get_salary, get_salary_range, get_experience, no_employee_error)
...
def find_employees_by_experience(employees: list) -> list:
    result = []
    lo, hi = get_experience()
    for employee in employees:
        if lo <= employee["experience"] <= hi:
            result.append(employee)
    return result
